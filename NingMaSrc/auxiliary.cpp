...
int AudioWav::read(const char* fname, char endian, int maxSamples)
{
  ...
  if (strcmp(szChunckID, "RIFF")) { 
    ...
    if ( strcmp(szChunckID, "WAVE"))
      ...
      int len = chunck_size / (bits_per_sample / 8);
		...
		cout << "bits per sample " << bits_per_sample << endl << endl;
		
	wavSize = len;
	//but don't necessarily want to read all these
	if (maxSamples > 0)
		len = std::min (len , maxSamples);
...
void AudioWav::anrPrint(){
	for (int i = 0 ; i < _data.size(); i++){
		cout << "vector " << i << " : " << _data[i] << endl;
	}
}
